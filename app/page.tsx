...
function layoutWarningIsCoveredByHealthPanel(warning: string): boolean {
  return warning.startsWith('Stability:') ||
    warning.startsWith('Stability V2:') ||
    warning.startsWith('Shadow compare');
}

function BuildHealth({ diagnostics }: { diagnostics: GeneratedModel['diagnostics'] }) {
  ...
  const readiness = floating > 0 || unsupported > unsupportedLimit || weak > weakLimit
    ? 'needs_repair'
